defmodule MakinaWeb.Stacks.ViewLive do
      ...
      <section>
        <.header level="h2" text_class="text-lg">
          ...
          <:actions>
            <.link
              ...
              navigate={~p"/stacks/#{@app.id}/services/create"}
            >
              <.icon name="hero-plus" />
            </.link>
          </:actions>
        ...
        <section :if={@app.services != []} class="grid grid-cols-4 gap-4 pt-5">
          <.card
            ...
            cta_url={~p"/stacks/#{@app.id}/services/#{service.id}"}
          >
            <:status_indicator>
  ...
  def mount(%{"id" => id}, _session, socket) do
    app = Stacks.get_app!(String.to_integer(id))

    if connected?(socket), do: PubSub.subscribe(Makina.PubSub, "stack::#{app.id}")
  ...
  def handle_event("create_api_token", _data, socket) do
    token = Stacks.create_api_token("api-token", socket.assigns.app)
